...
- (void)setupButtons
{
    ...
    [rightButton setTitle:D_LocalizedCardString(@"card_meitu_cancel") forState:UIControlStateNormal];
    [rightButton.titleLabel setFont:[UIFont fontWithName:fontName size:fontSize]];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/columns))
    {
        ...
        if (_numberOfVideos == 0)
            title = [NSString stringWithFormat:D_LocalizedCardString(@"PhotoHintHalf"), (long)_numberOfPhotos];
        else if (_numberOfPhotos == 0)
            title = [NSString stringWithFormat:D_LocalizedCardString(@"VideoHintHalf"), (long)_numberOfVideos];
        else
            title = [NSString stringWithFormat:D_LocalizedCardString(@"PhotoVideoHintHalf"), (long)_numberOfPhotos, (long)_numberOfVideos];
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        format = D_LocalizedCardString(@"PhotoVideoHint");
    
    else if (photosSelected)
        format = (indexPaths.count > 1) ? D_LocalizedCardString(@"PhotoHint") : D_LocalizedCardString(@"PhotoHint");
    
    else if (videoSelected)
        format = (indexPaths.count > 1) ? D_LocalizedCardString(@"VideoHint") : D_LocalizedCardString(@"VideoHint");
...
- (void)setupButtons
{
    ...
    if (picker.showCancelButton)
    {
        ...
        UIButton *rightButton  = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 70, 44)];
        [rightButton setTitle:D_LocalizedCardString(@"card_meitu_cancel") forState:UIControlStateNormal];
        [rightButton.titleLabel setFont:[UIFont fontWithName:fontName size:18]];
        [rightButton addTarget:self
...
- (void)showNotAllowed
{
    ...
    title.text              = D_LocalizedCardString(@"NoSupport");
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = D_LocalizedCardString(@"SettingHint");
...
- (void)showNoAssets
{
    ...
    title.text              = D_LocalizedCardString(@"NoExistedPhoto");
    title.font              = [UIFont systemFontOfSize:26.0];
    title.textColor         = [UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = D_LocalizedCardString(@"SyncPhoto");
