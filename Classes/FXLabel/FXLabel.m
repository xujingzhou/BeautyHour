          ...
          allowOrphans:(BOOL)allowOrphans
{
    if (lineSpacing || characterSpacing || [kerningTable count] || !allowOrphans || FXLABEL_MIN_TARGET_IOS7)
    {
        ...
        total.height = (MIN(size.height, [lines count] * font.lineHeight + ([lines count] - 1) * (font.pointSize *lineSpacing)));
                       ...
                       color:(UIColor *)color
{
    if (lineSpacing || characterSpacing || [kerningTable count] || !allowOrphans || FXLABEL_MIN_TARGET_IOS7)
    {
        ...
        for (NSString *line in lines)
        {
            ...
            {
                offset.x = (rect.origin.x + (rect.size.width - size.width)/ 2.0f);
            }
            else if (alignment == NSTextAlignmentRight)
            {
                offset.x = (rect.origin.x + rect.size.width - size.width);
            }
